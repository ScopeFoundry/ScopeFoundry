...
def mk_cell_0_source(files_metadata) -> List[str]:
    ...
    lines = (
        [
            "# CELL #0: AUTOGENERATED CELL: PROGRESS MAY BE LOST. use CELL #2 onwards",
            "# NOTE: In some scenarios, you need to open the root folder to make imports work. (File->Open Folder ... and choose the folder of this file)",
            "import numpy as np",
        ...
        + [
            "",
            '## HINT: to move the h5 files not mentioned in Cell#2 onwards to a subfolder "archived_data" run the following line:',
            "# loaders.archive_unmentioned_h5_files(ignore_cells=(0, 1), target_folder='archived_data')",
        ]
    )
...
def mk_cell_1_source(files_metadata: Dict) -> List[str]:
    return [
        "# CELL #1: Only addition and commenting of lines are persistent",
    ] + [mk_load_line(v) for v in files_metadata.values() if v["exists"]]


def update_cell_0(nb_content, files_metadata):
    nb_content["cells"][0]["source"] = mk_cell_0_source(files_metadata)
    nb_content["cells"][0]["execution_count"] = None


def update_cell_1(nb_content: Dict, files_metadata: Dict) -> Dict:
    for new_line in mk_cell_1_source(files_metadata):
        skipp = False
...
def update_ipynb(
    ...
    update_cell_0(nb_content, files_metadata)

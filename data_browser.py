...
class HyperSpectralBaseView(DataBrowserView):
    ...
    def setup(self):
        ...
        #settings
        self.default_display_image_choices = ['default', 'sum', 'median_map', 'bandpass_map']
        ...
        self.x_slice_LinearRegionItem.sigRegionChanged.connect(self.on_change_x_slice_LinearRegionItem)
        self.x_slice_LinearRegionItem.sigRegionChanged.connect(self.recalc_bandpass_map)
    ...
    def on_change_x_axis(self):
        ...
        if key in self.spec_x_arrays:
            ...
            # self.spec_plot.setLimits(xMin=self.spec_x_array.min(),
            #                         xMax=self.spec_x_array.max(),)
            self.spec_plot.setXRange(self.spec_x_array[0],
                                     self.spec_x_array[-1])
            self.update_display()
    ...
    def on_change_display_image(self):
        ...
        if self.display_image is None:
            self.databrowser.ui.statusbar.showMessage('No image to display!')
        elif self.display_image.shape == (1, 1):
            self.databrowser.ui.statusbar.showMessage('Cannot display single pixel image!')
        else:
            self.update_display()
    ...
    def on_change_rect_roi(self, roi=None):
        ...
        # NOTE: If data is indeed stored as (frame, y, x, time) in
        # self.hyperspec_data, then axis argument should be axes = (2,1)
        imitem = self.imview.getImageItem()
        if imitem is not None:
    ...
    def on_change_bg_subtract(self):
        ...
        if self.spec_x_array is not None:
            self.set_LinearRegionItem_activated(activate,
                                                self.bg_LinearRegionItem,
                                                [0.0, 0.03])
        self.update_display()
    ...
    def on_change_x_slice(self):
        ...
        if self.spec_x_array is not None:
            self.set_LinearRegionItem_activated(activate,
                                                self.x_slice_LinearRegionItem,
                                                [0.05, 1.0])

    def update_slice(self, slice_name, LinearRegionItem):
        '''
        ...
        '''
        mn,mx = LinearRegionItem.getRegion()
    ...
    def default_image_view(self):
        '''
        sets rect_roi congruent to imageItem and optimizes size of imageItem
        to fit the ViewBox
        '''
    ...
    def recalc_bandpass_map(self):
        x,hyperspec_data = self.get_xhyperspec_data(apply_use_x_slice=True)
        bandpass_map = hyperspec_data.sum(axis=-1)
        self.add_display_image('bandpass_map', bandpass_map)

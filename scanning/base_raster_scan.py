...
from ScopeFoundry import LQRange, Measurement
...
class BaseRaster2DScan(Measurement):
    ...
    def setup(self):
        ...
        h_lq_params = dict(
            vmin=self.h_limits[0],
            vmax=self.h_limits[1],
            unit=self.h_unit,
            spinbox_decimals=self.h_spinbox_decimals,
            spinbox_step=self.h_spinbox_step,
            dtype=float,
            ro=False,
        )
        h_range = self.h_limits[1] - self.h_limits[0]
        self.h0 = self.settings.New(
            "h0", initial=self.h_limits[0] + h_range * 0.25, **h_lq_params
        )
        self.h1 = self.settings.New(
            "h1", initial=self.h_limits[0] + h_range * 0.75, **h_lq_params
        )
        v_lq_params = dict(
            vmin=self.v_limits[0],
            vmax=self.v_limits[1],
            unit=self.v_unit,
            spinbox_decimals=self.v_spinbox_decimals,
            spinbox_step=self.v_spinbox_step,
            dtype=float,
            ro=False,
        )
        v_range = self.v_limits[1] - self.v_limits[0]
        self.v0 = self.settings.New(
            "v0", initial=self.v_limits[0] + v_range * 0.25, **v_lq_params
        )
        self.v1 = self.settings.New(
            "v1", initial=self.v_limits[0] + v_range * 0.75, **v_lq_params
        )

        lq_params = dict(
            dtype=float, vmin=1e-9, vmax=abs(h_range), ro=False, unit=self.h_unit
        )
        self.dh = self.settings.New("dh", initial=self.h_spinbox_step, **lq_params)
        self.dh.spinbox_decimals = self.h_spinbox_decimals
        lq_params = dict(
            dtype=float, vmin=1e-9, vmax=abs(v_range), ro=False, unit=self.v_unit
        )
        ...
        self.h_center = self.settings.New(
            "h_center",
            dtype=float,
            ro=False,
            unit=self.h_unit,
        ...
        self.v_center = self.settings.New(
            "v_center",
            dtype=float,
            ro=False,
            unit=self.v_unit,
        ...
        self.scan_type = self.settings.New(
            "scan_type",
            dtype=str,
            initial="raster",
            choices=(
        ...
        self.continuous_scan = self.settings.New(
            "continuous_scan", dtype=bool, initial=False
        )
        ...
        self.settings.New(
            "pixel_time", dtype=float, ro=True, si=True, initial=1, unit="s"
        )
        ...
        if not self.use_external_range_sync:
            self.h_range = LQRange(
                self.h0, self.h1, self.dh, self.Nh, self.h_center, self.h_span
            )
            self.v_range = LQRange(
                self.v0, self.v1, self.dv, self.Nv, self.v_center, self.v_span
            )
        ...
        # connect events
        self.settings.get_lq("show_previous_scans").add_listener(
            self.show_hide_previous_scans,
            argtype=(bool),
        )
        ...
        self.progress.connect_to_widget(self.ui.progressBar)

        self.settings.get_lq("continuous_scan").connect_to_widget(
            self.ui.continuous_scan_checkBox
        )
        self.settings.get_lq("save_h5").connect_to_widget(self.ui.save_h5_checkBox)

        self.settings.get_lq("show_previous_scans").connect_to_widget(
    ...
    def set_h_limits(self, vmin, vmax, set_scan_to_max=False):
        self.settings.get_lq("h0").change_min_max(vmin, vmax)
        self.settings.get_lq("h1").change_min_max(vmin, vmax)
        if set_scan_to_max:
            self.settings["h0"] = vmin
            self.settings["h1"] = vmax

    def set_v_limits(self, vmin, vmax, set_scan_to_max=False):
        self.settings.get_lq("v0").change_min_max(vmin, vmax)
        self.settings.get_lq("v1").change_min_max(vmin, vmax)
        if set_scan_to_max:
    ...
    def pre_run(self):
        ...
        for lqname in "h0 h1 v0 v1 dh dv Nh Nv".split():
            self.settings.get_lq(lqname).change_readonly(True)
    ...
    def post_run(self):
        ...
        for lqname in "h0 h1 v0 v1 dh dv Nh Nv".split():
            self.settings.get_lq(lqname).change_readonly(False)
    ...
    def setup_figure(self):
        ...
        self.img_plot: pg.PlotItem = self.graph_layout.addPlot()
        ...
        for lqname in "h0 h1 v0 v1 dh dv".split():
            self.settings.get_lq(lqname).updated_value.connect(self.update_scan_roi)
    ...
    def disconnect_pos_widgets(self):
        if hasattr(self, "stage"):
            s: LQCollection = self.stage.settings
            s.get_lq("x_position").disconnect_from_widget(self.ui.x_doubleSpinBox)
            s.get_lq("y_position").disconnect_from_widget(self.ui.y_doubleSpinBox)
    ...
    def connect_pos_widgets(self):
        ...
        if hasattr(self, "stage"):
            ...
            s.get_lq("x_position").updated_value.connect(
                self.update_arrow_pos, QtCore.Qt.UniqueConnection
            )
            s.get_lq("y_position").updated_value.connect(
                self.update_arrow_pos, QtCore.Qt.UniqueConnection
            )

            s.get_lq("x_position").connect_to_widget(self.ui.x_doubleSpinBox)
            s.get_lq("y_position").connect_to_widget(self.ui.y_doubleSpinBox)
    ...
    def update_display(self):
        ...
        if self.initial_scan_setup_plotting:
            ...
            x0, x1, y0, y1 = self.imshow_extent
            self.log.debug(f"update_display set bounds {x0} {x1} {y0} {y1}")
            self.img_item_rect = QtCore.QRectF(x0, y0, x1 - x0, y1 - y0)
            self.img_item.setRect(self.img_item_rect)
            self.log.debug(f"update_display set bounds {self.img_item_rect}")
    ...
    def mouseMoved(self, evt):
        ...
        self.pos_label.setText(
            f"H {mousePoint.x():+02.2f} um [{0}], V {mousePoint.y():+02.2f} um [{0}]: {0:1.2e} Hz"
        )
    ...
    def scan_specific_setup(self):
        ...
        # self.stage.settings.get_lq("x_position").connect_to_widget(self.ui.x_doubleSpinBox)
        # self.stage.settings.get_lq("y_position").connect_to_widget(self.ui.y_doubleSpinBox)
...
class BaseRaster3DScan(Measurement):
    ...
    def setup(self):
        ...
        z_lq_params = dict(
            ...
            unit=self.z_unit,
            spinbox_decimals=self.z_spinbox_decimals,
            spinbox_step=self.z_spinbox_step,
            dtype=float,
            ro=False,
        )
        z_range = self.z_limits[1] - self.z_limits[0]
        self.z0 = self.settings.New(
            "z0", initial=self.z_limits[0] + z_range * 0.25, **z_lq_params
        )
        self.z1 = self.settings.New(
            "z1", initial=self.z_limits[0] + z_range * 0.75, **z_lq_params
        )
        ...
        self.dv.spinbox_decimals = self.v_spinbox_decimals
        lq_params = dict(
            dtype=float, vmin=1e-9, vmax=abs(z_range), ro=False, unit=self.z_unit
        )
        ...
        self.z_center = self.settings.New(
            "z_center",
            dtype=float,
            ro=False,
            unit=self.z_unit,
        ...
        self.scan_type = self.settings.New(
            "scan_type", dtype=str, initial="raster", choices=("raster", "serpentine")
        )
        ...
        if not self.use_external_range_sync:
            ...
            self.z_range = LQRange(
                self.z0, self.z1, self.dz, self.Nz, self.z_center, self.z_span
            )
    ...
    def set_z_limits(self, zmin, zmax, set_scan_to_max=False):
        self.settings.get_lq("z0").change_min_max(zmin, zmax)
        self.settings.get_lq("z1").change_min_max(zmin, zmax)
        if set_scan_to_max:
    ...
    def disconnect_pos_widgets(self):
        if hasattr(self, "stage"):
            ...
            s.get_lq("x_position").disconnect_from_widget(self.ui.x_doubleSpinBox)
            s.get_lq("y_position").disconnect_from_widget(self.ui.y_doubleSpinBox)
            s.get_lq("z_position").disconnect_from_widget(self.ui.y_doubleSpinBox)

    def connect_pos_widgets(self):
        self.disconnect_pos_widgets()

        if hasattr(self, "stage"):
            print(self.name, "connect_pos_widgets")
            s: LQCollection = self.stage.settings

            s.get_lq("x_position").updated_value.connect(self.update_arrow_pos)
            s.get_lq("y_position").updated_value.connect(self.update_arrow_pos)

            s.get_lq("x_position").connect_to_widget(self.ui.x_doubleSpinBox)
            s.get_lq("y_position").connect_to_widget(self.ui.y_doubleSpinBox)
            s.get_lq("z_position").connect_to_widget(self.ui.z_doubleSpinBox)
    ...
    def scan_specific_setup(self):
        ...
        # self.app.hardware_components['dummy_xy_stage'].get_lq("x_position").connect_to_widget(self.ui.x_doubleSpinBox)
        # self.app.hardware_components['dummy_xy_stage'].get_lq("y_position").connect_to_widget(self.ui.y_doubleSpinBox)
    ...
    def gen_raster_scan(self, gen_arrays=True):
        ...
        if gen_arrays:
            ...
            JJ, KK, II = np.meshgrid(
                np.arange(self.Nv.val), np.arange(self.Nz.val), np.arange(self.Nh.val)
            )
